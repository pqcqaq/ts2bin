...
| `FND-004` | 建立可获取的 ts2bin fork/patch 交付与 clean-clone 门禁 | FND-001 | lock 固定 upstream 与 fork commit 或 patch base/path/hash；doctor 要求 materialized-exact；remote clean checkout apply 后通过测试/vet |
...
| `IR-000` | SourceTypePlan -> typed HIR -> specialization fixed point -> target RepPlan -> MIR CFG/SSA -> verifier 的唯一 pass/effect DAG | FE-009, FE-010, FE-011 | 文档/实现/IR golden 使用同一顺序；循环实例化、effect 和 malformed pass 输入可验证 |

### 3.2 当前审计状态（2026-08-05）

| ID | 状态 | 已验证结果 / 剩余工作 |
| --- | --- | --- |
| `FND-004` | `implemented / acceptance-blocked` | 最终 patch/hash 已重建，`doctor -Quiet` 为 materialized-exact，官方 remote shallow clean checkout apply 后 full test/vet/cleanup 通过，WSL Go-LLVM/Rust+LLD smoke 通过；parent HEAD 尚未获授权提交正确 gitlink/lock/patch/scripts，因此 clean-parent revision 证明仍未严格关闭。 |
...
`FE-008a/009a/010a/011a/011b` 与 `IR-000a` 已关闭，不再重做已出现的 registry/executor。最终 patch/hash、doctor、官方 remote clean checkout full test/vet 和 WSL smoke 也已通过；`FND-004a` 只剩获授权的 parent commit/HEAD clean-clone 证明。之后 Phase 2A 先并行完成 `IR-007a` number 契约、`BE-001a` Go-LLVM/TargetMachine 基座与 `RT-002a` Rust workspace/startup scaffold，再由 `TC-001a` 把 canonical BuildPlan request 绑定为可执行 TargetContext；随后实现 `IR-001a..005a`、`RT-002b`、`BE-002a/004a`、`REL-001a`、`VERT-001` 和 `REL-002a`。只有这条纵切通过后，才进入控制流和广泛语法的 Phase 2B。

| 子任务 | 必须交付的关闭证据 |
| --- | --- |
| `FND-004a` | patch SHA-256 `759e0661a91c7b757a78106425618046dc0b8e348f2c1e31263f486c074a9c9f`、doctor materialized-exact、官方 remote shallow clean checkout apply/full test/vet/cleanup 与 WSL smoke 已通过；剩余关闭证据是 parent HEAD 包含正确 gitlink、lock、patch、materialize/doctor/verify scripts，并从该 committed clean parent clone 重跑。 |
...
| ID | 结果 | 依赖 | 主要验收 |
| --- | --- | --- | --- |
| `IR-001` | 固定 TsType/RepType/entity/origin schema v1 | IR-000, FE-009, FE-010 | schema round-trip、版本拒绝、snapshot-only input 和 canonical hash 测试 |
...
| --- | --- | --- |
| `IR-007a` | IR-000a, FND-004a | 在 RepType schema 前冻结 JavaScript `number = f64`、NaN payload policy、`-0`、`+` operator 和 C ABI bit-observation contract；不提前接受其他 primitive representation |
| `IR-001a` | IR-000a, IR-007a, FE-009a, FE-010a, FE-011b, FND-004a | 冻结只允许 number/void 的 TsType/RepType/entity/origin schema；bool/string/null/undefined fail closed |
| `IR-002a` | IR-001a | serialized snapshot 只生成参数读取、number 加法、单 block return 的 canonical HIR |
| `IR-003a` | IR-002a | HIR verifier 拒绝篡改 schema、ID、type/effect/origin/terminator、非 number op 和 unsupported CFG |
| `BE-001a` | FND-004a | 建立 Go-LLVM wrapper、锁定 Linux x86-64 TargetMachine/DataLayout，并让独立最小 module 通过 VerifyModule/object emission；不依赖完整 MIR |
| `RT-002a` | FND-004a | 建立 Cargo workspace、empty startup、最小 umbrella staticlib 和 toolchain/runtime manifest scaffold；不伪造尚未绑定的 MIR ABI/capability |
| `TC-001a` | FE-011b, BE-001a, RT-002a | 将 canonical unresolved BuildPlan request 与 toolchain/runtime manifest 绑定为 TargetContext + DataLayout + CapabilitySet；首切只接受显式 Linux x86-64、LLVM 20、generic CPU、no-EH 和锁定 runtime，空 target、interop/unsafe、ARC/arena、bounds-off、未知 feature/runtime fail closed |
| `IR-004a` | IR-003a, TC-001a | 只消费已绑定 TargetContext，将 first-slice HIR 降为 target-aware 单 block MIR；无 placeholder store/phi/call |
| `IR-005a` | IR-004a, TC-001a | MIR verifier 独立验证 dense/unique IDs、RepType/DataLayout、return/effect/provenance/capability，malformed MIR 不进入 backend |
...
| `VERT-001` | Linux x86-64 real-LLVM executable vertical slice (`add(number, number)`) | FND-004a, FE-008a, FE-009a, FE-010a, FE-011a, FE-011b, IR-000a, TC-001a, IR-005a, RT-002b, BE-004a, REL-001a | case runner 执行 serialized snapshot-only lowering -> HIR/MIR verifier -> bound TargetContext -> real LLVM -> object -> LLD -> run；由固定 C ABI harness 输出 IEEE-754 bits，不含对象、GC、EH、字符串 |
...
1. `FE-008a/009a/010a/011a/011b`、`IR-000a`：冻结 lowering-complete snapshot、语义 proof、snapshot-only replay、truthful target/profile provenance 和唯一 pass contract。
2. `FND-004a`：最终 patch/hash 与官方 remote clean checkout full test/vet 已通过；获授权后提交正确 parent gitlink/lock/patch/scripts，并从该 parent HEAD clean clone 复验，不依赖未记录的 submodule dirty state。
3. `IR-007a` 先于 `IR-001a` 冻结 f64/NaN/-0/`+`；`BE-001a` 与 `RT-002a` 可并行建立 TargetMachine/DataLayout 和 Rust workspace/startup scaffold。
4. `TC-001a` 把 BuildPlan request 绑定为可执行 TargetContext 后，`IR-001a..005a` 只实现 primitive `number`/参数/返回/加法的可验证 HIR/MIR；`RT-002b`、`BE-002a/004a` 再完成固定 C ABI、真实 LLVM、object 和 LLD 链接能力。
