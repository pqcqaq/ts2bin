...
- Phase 1.5 已形成 schema v2、wire 单一 validator、Kind shape/semantic-proof registry、带 provenance 的 target-independent `FrontendSnapshot`、canonical unresolved `BuildPlan`、checker-free replay，以及执行到 typed HIR 的 canonical production pass 前缀。`FE-012a`、`IR-007a/001a/002a/003a/004a/005a/008a`、`BE-001a`、`RT-002a` 与 `TC-001a` 已关闭：LLVM 20 TargetMachine/DataLayout、Rust runtime manifests、strict resolver、immutable TargetContext、RepresentationPlan join、target-aware f64 MIR、显式空 BoundCapabilityClosure、canonical HIR/MIR emit/verify/diff CLI 与 first-slice case 均已有真实证据。下一项是固定 C ABI、真实 LLVM lowering、object/LLD 与 runner；在 number-only 可执行纵切通过前，Phase 2B 和广泛语法保持 blocked。
