...
- Phase 1.5 已形成 schema v2、wire 单一 validator、Kind shape/semantic-proof registry、带 provenance 的 target-independent `FrontendSnapshot`、canonical unresolved `BuildPlan`、checker-free replay，以及执行到 typed HIR 的 canonical production pass 前缀。`FE-012a`、`IR-007a/001a/002a/003a/004a/005a/008a`、`BE-001a/002a`、`RT-002a/002b` 与 `TC-001a` 已关闭：LLVM 20 TargetMachine/DataLayout、strict resolver、target-aware f64 MIR、固定 C ABI bit harness、真实 LLVM verifier/object emission、canonical HIR/MIR CLI 与 first-slice case 均已有真实证据。下一项是确定性 LLD link/run probe 与 runner；在 number-only 可执行纵切通过前，Phase 2B 和广泛语法保持 blocked。
