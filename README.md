...
`ts2bin` is a TypeScript-to-native compiler project built around `typescript-go`, a target-independent frontend wire snapshot, Bingo typed HIR, target-aware MIR, LLVM, and a Rust C ABI runtime. The executable boundary is `FrontendSnapshot -> typed HIR`; `BuildPlan + manifests -> ResolveTargetContext`; then `RepresentationPlan -> target-aware MIR -> LLVM/link`. Phase 1.5 and the number-only Phase 2A vertical slice are complete on the pinned fork: the real LLVM 20 path emits and links a Linux x86-64 executable, runs it, and differentially verifies ordinary, `-0`, and canonical qNaN results against Node 22.22.0. Phase 2B now has two executable vertical slices: strict-boolean `choose` and `calllocal` with SSA local binding/assignment, a signature-bound direct call to an internal helper, multi-function LLVM/object/link/process provenance, and Node differential. The next slice is loop/general CFG and SSA/phi; string/nullish representation and single-evaluation lowering follow. Object/runtime-heavy features and self-hosted stdlib remain in later phases. See [the audit](plans/development-audit-2026-08-05.md) and [implementation backlog](plans/implementation-backlog.md).
