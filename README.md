...
`ts2bin` is a TypeScript-to-native compiler project built around `typescript-go`, a target-independent frontend wire snapshot, Bingo typed HIR, target-aware MIR, LLVM, and a Rust C ABI runtime. The executable boundary is `FrontendSnapshot -> typed HIR`; `BuildPlan + manifests -> ResolveTargetContext`; then `RepresentationPlan -> target-aware MIR -> LLVM/link`. The direction audit kept that architecture and added a Phase 1.5 lowering-contract gate. The Phase 1.5 implementation contracts and Phase 2A entry contracts (`FE-012a`, `IR-007a`, `IR-001a..005a`, `IR-008a`) are complete; delivery acceptance (`FND-004a`) is complete on the pinned fork and committed parent gitlink. P2A now has a checker-free case manifest, canonical HIR/MIR emit/verify/diff CLI, and a real LLVM 20 MIR output gate. Broad syntax expansion stays blocked until the number-only real-LLVM executable slice passes. See [the audit](plans/development-audit-2026-08-05.md) and [implementation backlog](plans/implementation-backlog.md).
