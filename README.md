...
`ts2bin` is a TypeScript-to-native compiler project built around `typescript-go`, a target-independent frontend wire snapshot, Bingo typed HIR, target-aware MIR, LLVM, and a Rust C ABI runtime. The executable boundary is `FrontendSnapshot -> typed HIR`; `BuildPlan + manifests -> ResolveTargetContext`; then `RepresentationPlan -> target-aware MIR -> LLVM/link`. Phase 1.5 and the number-only Phase 2A vertical slice are complete on the pinned fork: the real LLVM 20 path emits and links a Linux x86-64 executable, runs it, and differentially verifies ordinary, `-0`, and canonical qNaN results against Node 22.22.0. Phase 2B now has three executable vertical slices: strict-boolean `choose`, local binding/direct call in `calllocal`, and `while` loop/general CFG with edge-aware SSA phi nodes. All three run as real deterministic ELF artifacts and are checked against Node. The next slices freeze string/nullish representation and single-evaluation lowering. Object/runtime-heavy features and the self-hosted standard-library package remain in later phases. Compiler self-bootstrap is not part of the current roadmap because the driver, frontend integration, and lowering are implemented in Go. See [the audit](plans/development-audit-2026-08-05.md) and [implementation backlog](plans/implementation-backlog.md).
