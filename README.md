...
`ts2bin` is a TypeScript-to-native compiler project built around `typescript-go`, a target-independent frontend wire snapshot, Bingo typed HIR, target-aware MIR, LLVM, and a Rust C ABI runtime. The executable boundary is `FrontendSnapshot -> typed HIR`; `BuildPlan + manifests -> ResolveTargetContext`; then `RepresentationPlan -> target-aware MIR -> LLVM/link`. Phase 1.5, Phase 2A, and the scoped Phase 2B static-core milestone are complete on the pinned fork. Phase 2B contains seven real Linux x86-64 LLVM/object/LLD/ELF/Node vertical slices (`choose`, `classify`, `calllocal`, `loop`, `coalesce`, local `coalesceAssign`, and borrowed UTF-16 `stringLength`) plus a real-source `ts2bin build` preview for exactly one exported `main(): number` returning a canonical integer literal from 0 through 255. The preview emits a deterministic Linux x86-64 ELF and adjacent provenance report; it is deliberately not the Phase 6 general product backend. Property/computed-key/getter/optional-chain place evaluation remains open for Phase 3, while owned strings, allocation/GC, modules, and the self-hosted standard-library package remain in later phases. Compiler self-bootstrap is not part of the current roadmap because the driver, frontend integration, and lowering are implemented in Go. See [the audit](plans/development-audit-2026-08-05.md) and [implementation backlog](plans/implementation-backlog.md).
