...
`ts2bin` is a TypeScript-to-native compiler project built around `typescript-go`, a target-independent frontend wire snapshot, Bingo typed HIR, target-aware MIR, LLVM, and a Rust C ABI runtime. The executable boundary is `FrontendSnapshot -> typed HIR`; `BuildPlan + manifests -> ResolveTargetContext`; then `RepresentationPlan -> target-aware MIR -> LLVM/link`. Phase 1.5 and the number-only Phase 2A vertical slice are complete on the pinned fork. Phase 2B now has seven real Linux x86-64 LLVM/object/LLD/ELF/Node vertical slices: strict-boolean `choose`, numeric-literal/multi-return `classify`, local binding/direct call in `calllocal`, `while` loop/general CFG with edge-aware SSA phi nodes, nullable-number `coalesce`, local nullable `coalesceAssign`, and borrowed immutable UTF-16 `stringLength`. The current CLI still exposes these through the static-core case runner rather than a general `build` command; an explicitly restricted application entrypoint/build preview is a separate Phase 2B exit milestone. Property/computed-key/getter/optional-chain place evaluation remains open for Phase 3, while owned strings, allocation/GC, modules, and the self-hosted standard-library package remain in later phases. Compiler self-bootstrap is not part of the current roadmap because the driver, frontend integration, and lowering are implemented in Go. See [the audit](plans/development-audit-2026-08-05.md) and [implementation backlog](plans/implementation-backlog.md).
