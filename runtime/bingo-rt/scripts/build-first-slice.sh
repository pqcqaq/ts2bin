...
python3 "$ROOT/tools/write_runtime_manifest.py" --archive "$ARCHIVE" --startup "$STARTUP" --harness "$HARNESS" --compute-harness "$COMPUTE_HARNESS" --choose-harness "$CHOOSE_HARNESS" --output "$MANIFEST"
"$CLANG" -target "$TRIPLE" -I"$ROOT/include" "$ROOT/tests/link_smoke.c" "$STARTUP" "$ARCHIVE" -ldl -lpthread -lm -o "$SMOKE"
"$SMOKE"

printf 'umbrella=%s\nstartup=%s\nharness=%s\ncompute_harness=%s\nchoose_harness=%s\nmanifest=%s\nsmoke=%s\n' "$ARCHIVE" "$STARTUP" "$HARNESS" "$COMPUTE_HARNESS" "$CHOOSE_HARNESS" "$MANIFEST" "$SMOKE"
